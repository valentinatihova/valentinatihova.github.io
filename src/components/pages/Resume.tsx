...
const SectionHeader = ({ num, label }: { num: string; label: string }) => (
  <div className="flex items-center gap-6 mb-10 border-t border-stone-200 pt-10">
    <div className="shrink-0">
      <p className="font-mono text-[10px] tracking-[0.22em] text-stone-400 mb-1">{num}</p>
    ...
    if (!previewUrl) {
      return (
        <div className="flex min-h-[320px] items-center justify-center border border-dashed border-stone-300 bg-stone-50 p-8 text-center">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.24em] text-stone-400">Preview not attached</p>
      ...
      <div className="container mx-auto px-4 pt-16 pb-24 max-w-5xl">
        ...
        <div className="mb-16">
          ...
          <div className="flex items-center gap-4 mb-10">
            ...
            <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-stone-400">01 — Resume / CV</p>
          </div>
        ...
        <div className="border-y border-stone-200 py-5 mb-16">
          <div className="flex flex-wrap gap-x-6 gap-y-1.5 font-mono text-[10px] uppercase tracking-[0.18em] text-stone-400">
        ...
        <section className="mb-16">
          <p className="font-mono text-[10px] uppercase tracking-[0.28em] text-stone-400 mb-4">
        ...
        <section className="mb-16">
          ...
          <p className="mb-10 max-w-[62ch] font-serif text-sm text-stone-400 leading-relaxed">
          ...
          <div className="divide-y divide-stone-200 border-t border-stone-200">
            {profile.experience.map((exp, index) => (
              ...
              >
                <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-stone-400 md:pt-1">
                  {exp.period}
                </div>
                <div>
                  ...
                  <div className="font-mono text-[11px] uppercase tracking-[0.15em] text-stone-400 mb-4">
                    {exp.company}
                  </div>
                  ...
                  {exp.tools && (
                    <div className="flex flex-wrap gap-x-3 gap-y-1.5 font-mono text-[10px] uppercase tracking-[0.15em] text-stone-400">
            ...
            {profile.certifications?.map((cert, index) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="shrink-0 font-mono text-[10px] uppercase tracking-[0.15em] text-stone-400">{cert.date}</span>
                </div>
                <div className="font-mono text-[10px] uppercase tracking-[0.15em] text-stone-400 mb-3">{cert.issuer}</div>
                {cert.details && (
                  <p className="font-serif text-sm text-stone-500 border-t border-stone-100 pt-3 leading-relaxed">
                    {cert.details}
                  </p>
                )}
                <div className="mt-4 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.18em] text-stone-400 transition-colors group-hover:text-stone-900">
                  Preview certificate
          ...
          <div className="divide-y divide-stone-200 border-t border-stone-200">
            {profile.education.map((edu, index) => (
              <div key={index} className="grid gap-3 py-7 md:grid-cols-[9rem_1fr] md:gap-10">
                <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-stone-400">{edu.period}</span>
                <div>
                  <h4 className="font-serif text-lg text-stone-900">{edu.degree}</h4>
                  <div className="mt-1 font-mono text-[11px] uppercase tracking-[0.15em] text-stone-400">{edu.school}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <>
            ...
            >
              <div className="flex items-start justify-between gap-4 border-b border-stone-200 px-6 py-5">
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-[0.28em] text-stone-400 mb-2">Certificate preview</p>
                  <h3 className="font-serif text-xl text-stone-900">{activeCertification.name}</h3>
                  <p className="mt-1.5 font-mono text-[10px] uppercase tracking-[0.15em] text-stone-400">
